...
def test_net(cfg, epoch_idx=-1, test_data_loader=None, test_writer=None, grnet=None):
    ...
    if test_data_loader is None:
        # Set up data loader
        dataset_loader = grnet_point_cloud_completion.utils.data_loaders.DATASET_LOADER_MAPPING[cfg.DATASET.TEST_DATASET](cfg)
    ...
    #datas = dataset_loader.get_dataset(utils.data_loaders.DatasetSubset.TEST)
        ...
        try:
            ...
            with torch.no_grad():
                ...
                if test_writer is not None and model_idx < 3:
                    ...
                    sparse_ptcloud_img = grnet_point_cloud_completion.utils.helpers.get_ptcloud_img(sparse_ptcloud)
                    test_writer.add_image('Model%02d/SparseReconstruction' % model_idx, sparse_ptcloud_img,
                                          epoch_idx)
                    dense_ptcloud = dense_ptcloud.squeeze().cpu().numpy()
                    dense_ptcloud_img = grnet_point_cloud_completion.utils.helpers.get_ptcloud_img(dense_ptcloud)
                    ...
                    gt_ptcloud_img = grnet_point_cloud_completion.utils.helpers.get_ptcloud_img(gt_ptcloud)

    ...
    for subset, loader in test_loaders.items():
        ...
        for batch in tqdm(loader):
            ...
            with torch.no_grad():
                ...
                post_pred = nnf.interpolate(post_pred, size=(256,144))

                batch["gt_depth"] = nnf.interpolate(batch["gt_depth"], size=(256,144))
                batch['gt_depth'][batch['gt_depth']!= batch['gt_depth']] = 0
                # batch["gt_depth"] = torch.nan_to_num(batch['gt_depth'],nan = 0.0)
                # print(batch['gt_depth'])
                mask_valid_region = (batch['gt_depth'] > 0).byte()
                mask_seg = (nnf.interpolate(batch["mask"], size=(256,144)) <=0).byte()
                # print('mask valid region', torch.sum(mask_valid_region))
                # print('mask seg',torch.sum(mask_seg))
                # print('grnet', torch.sum((post_pred>0.0).byte()))
                mask_bool = mask_valid_region.__and__(mask_seg)
                no_empty = False
                if no_empty: 
                    mask_bool = mask_bool.__and__((post_pred>0.0).byte())
                # print('mask seg',torch.sum(mask_bool))
                # mask_bool = torch.logical_and(mask_valid_region,mask_seg).byte()
                rmse_temp_val = ((batch["gt_depth"][mask_bool] - post_pred[mask_bool])**2).mean().sqrt().cpu().detach().numpy()
                if rmse_temp_val == rmse_temp_val:

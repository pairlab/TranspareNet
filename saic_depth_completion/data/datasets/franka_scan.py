...
class FrankaScan:
    def __init__(
            self, root=ROOT, split="train", transforms=None, processed=True, model_segment=True,pcc_trans10k = False, remove_tag = False, size = None
    ):
        '''
        Input:
            ...
            processed: bool
                If True, will use the PCC output
            model_segment: bool
                If True, will use the Trans10k produced segmentation mask
            pcc_trans10k: bool
                If True, will use the PCC output when the mask is based on Trans10k
            remove_tag: bool
                If True, will remove the april tags from the RGB stream, and replace with gray pixels.
            size: list[y,x] or None
                If not None, will resize the images, depth, and mask to [y,x] dimension

        
        '''
        print('ROOT', root)
        self.transforms = transforms
        self.data_root = os.path.join(root,split) # os.path.join(root, "data")
        self.size = size
    ...
    def _load_data(self,processed=True,pcc_trans10k = False):
    ...
    def __getitem__(self, index):
        ...
        if self.size:
            ...
            gt_mask = resize(gt_mask,(240,320))
        return  {

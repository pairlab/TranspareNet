...
class DepthRel_test(nn.Module):
    def __init__(self, eps=1e-5):
        super(DepthRel_test, self).__init__()
        self.eps = eps
    def forward(self, pred, gt):
        mask = gt > self.eps
        # diff = torch.abs(gt[mask] - pred[mask]) / gt[mask]
        diff = torch.abs(gt - pred) / gt
        return diff.median()

class DepthRelAvg_test(nn.Module):
    def __init__(self, eps=1e-5):
        super(DepthRelAvg_test, self).__init__()
        self.eps = eps
    def forward(self, pred, gt):
        mask = gt > self.eps
        # diff = torch.abs(gt[mask] - pred[mask]) / gt[mask]
...
class SSIM_test(torch.nn.Module):
    def __init__(self, window_size=11, size_average=True, eps=1e-5):
        super(SSIM_test, self).__init__()
        self.eps = eps
